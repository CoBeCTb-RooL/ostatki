...
$currentRegionId = $MODEL['currentRegionId'];
$error = $MODEL['error'];
...
<?if(!$MODEL['error']):?>
	<div class="view">
		<h1><?=$title?></h1>
		<form id="form" method="" action="/<?=ADMIN_URL_SIGN?>/geo/cityEditSubmit" onsubmit="Geo.City.editSubmit(); /*return false;*/ " target="frame7">
            ...
            <div class="field-wrapper">
                <span class="label">Регион<span class="required">*</span>: </span>
                <span class="value">
                    <select name="regionId" >
                        <option value="">-выберите-</option>
                        <?foreach ($countries as $c):?>
                        <option disabled="disabled" ><?=$c->name?> <?=$c->status->code == Status::INACTIVE ? '(НЕАКТ)' : ''?></option>

                            <!--форичим регионы-->
                            <?
                            $regions = $c->regions($regionsDict);
                            ?>
                            <?foreach ($regions as $r):?>
                                <option value="<?=$r->id?>" <?=$r->id == $item->regionId ? ' selected="selected" ' : ($r->id == $currentRegionId ? ' selected="selected" ' : '') ?>   >&nbsp;&nbsp;&nbsp;<?=$r->name?> <?=$r->status->code == Status::INACTIVE ? '(НЕАКТ.)' : ''?></option>
                            <?endforeach;?>
