...
var Country = {
    ...
    switchStatus: function(id){
        $.ajax({
            url: '/<?=ADMIN_URL_SIGN?>/geo/switchCountryStatus',
            data: 'id='+id,
            dataType: 'json',
            beforeSend: function(){$.fancybox.showLoading()},
            success: function(data){
                if(data.error==''){
                    $('.country-'+id).removeClass('status-1').removeClass('status-2').addClass('status-'+data.status.num)
                }
                else
...
function switchStatus(id)
{
	//alert(id); return
	$.ajax({
		url: '/<?=ADMIN_URL_SIGN?>/geo/switchStatus',
		data: 'id='+id,
		dataType: 'json',
		beforeSend: function(){$.fancybox.showLoading()},
		success: function(data){
...
function saveCity()
{
	...
	$.ajax({
		url: '/<?=ADMIN_URL_SIGN?>/geo/editSubmit',
...
<h1><i class="fa fa-globe"></i> Города и страны</h1>
