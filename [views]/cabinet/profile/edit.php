...
<div class="cabinet" >	
	...
	<h2><?=($user ? 'РЕДАКТИРОВАНИЕ ЛИЧНЫХ ДАННЫХ' : 'РЕГИСТРАЦИЯ')?></h2>
		...
		<form  id="edit-form" method="post" action="/cabinet/profile/profileEditSubmit" target="frame6" onsubmit="return checkProfileForm(<?=$user?'false':'true'?>); ">
			...
			if(!$user)
			{?>
				<div style="margin: 0 0 10px 13px ">Начните работу с сайтом <strong class="red"><?=DOMAIN_CAPITAL?></strong>, пройдя несложную регистрацию. Это позволит Вам подавать в систему <b>свои объявления</b>, а также расширит возможности использования портала.<p>Итак, приступим! </div>
			<?php 
			}?>
...
<!--                <hr>-->
            ...
            if($user)
            {
                $geo = new GeoHelper($user ? $user->cityId : $_SESSION['city']->id);
            }
            ?>
